<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<link rel="stylesheet" href="assets/css/registerStyle.css"></link>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <title>User Registration</title>
    <style>
    
    </style>
</head>
<body>
    <div class="mydiv1">
       <center> <div class="mydiv2">
        <img src="assets/image/pic4.png" id="user-img">
        User Registration
        <div class="mydiv3">
            <div class="inner">
                <i class="fa fa-user fa-lg"></i>
            <input type="text" class="input" placeholder="user name">
            </div>
           
            <div class="inner">
                <i class="fa fa-envelope icon"></i>
                <input   type="email"   class="input" placeholder=" enter email">
            </div>

           <div class="inner">
            <i class="fa fa-key fa-lg"></i>
            <input type="password"  class="input" placeholder=" Enter password">
           </div>
           <div class="inner">
            <i class="fa fa-key fa-lg"></i>
            <input type="password"  class="input" placeholder=" Re-enter password">
           </div>
            
            <div class="btn-div">
                <button type="submit" class="btn">Registration</button>
            </div>
            <br>
            <span> Already have Account ? <a href="assets/html/Login.html" >  Sign In </a></span>
        </div>
      
        <div id="social">
          <a href="#" class="fa fa-facebook"></a>
          <a href="#" class="fa fa-twitter"></a>
          <a href="#" class="fa fa-google"></a>
          <a href="#" class="fa fa-linkedin"></a>
            </div>
     
        </div>


    </div>
          </center>
          
    </div>
</body>
</html>